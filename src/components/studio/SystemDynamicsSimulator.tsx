...
function ModelCanvas({
  ...
  const handleMouseUp = (_e: React.MouseEvent, targetNodeId: string | null = null) => {
  ...
  const getNodeColor = (type: string) => {
    switch (type) {
      ...
      default:
        return { bg: "#6B7280", border: "#374151" };
    }
  };
      ...
      {nodes.map((node) => {
        ...
        return (
          ...
          >
            ...
            />
            <text x="0" y="-15" textAnchor="middle" fill="white" fontSize="20">
              {getNodeIcon(node.type)}
            </text>
            <text x="0" y="5" textAnchor="middle" fill="white" fontSize="11" fontWeight="bold">
              {node.type.toUpperCase()}
            </text>
            <text x="0" y="20" textAnchor="middle" fill="white" fontSize="10">
              {node.data.label}
            </text>
          </g>
        );
  ...
  const runSimulation = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        links,
        config: {
          start_time: simConfig.startTime,
          end_time: simConfig.endTime,
          time_step: simConfig.timeStep,
          method: 'euler',
        },
      };
...
reader.onload = (e) => {
  try {
    ...
    if (typeof result === "string") {
      ...
      setError('File data was not text. Unable to load model.');
    }
  } catch (error: any) {
          ...
          <div className="space-y-2">
            <div>
              ...
              <input
                ...
                onChange={(e) => setSimConfig({ ...simConfig, startTime: Number(e.target.value) })}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setSimConfig({ ...simConfig, endTime: Number(e.target.value) })}
                className="w-full px-2 py-1 border rounded text-sm"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setSimConfig({ ...simConfig, timeStep: Number(e.target.value) })}
                className="w-full px-2 py-1 border rounded text-sm"
              />
            </div>
          </div>
...
export default SystemDynamicsSimulator;